<script>
    document.addEventListener('DOMContentLoaded', () => {
			  ...
			  // Function to detect if Elementor editor is active
        function isInElementorEditor() {
            return (
                body.classList.contains('elementor-editor-active') || 
                body.classList.contains('e-route-panel-editor-layout') // Check for another Elementor editor indicator
            );
        }
			  // Check if we are not in Elementor editor mode
			  function checkElementorEditor() {
						if (isInElementorEditor()) {
							 // Apply overflow hidden if not in editor
			  ...
			  checkElementorEditor();
			 	...
			 	// Use MutationObserver to detect any dynamic changes in body classes
